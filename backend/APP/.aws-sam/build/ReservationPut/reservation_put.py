...
def put_customer_reservation_info(body, shop_info, message_id1, message_id2):
    """
...
def put_shop_reservation_info(body, shop_info):
    ...
    openTime = datetime.datetime.strptime(shop_info["shop"]["openTime"], "%H:%M")
    closeTime = datetime.datetime.strptime(shop_info["shop"]["closeTime"], "%H:%M")
    restaurant_open_term = int((closeTime - openTime) / THIRTY_MINUTES)
    max_reservable_number = int(shop_info["shop"]["seatsNumber"]) * restaurant_open_term
    ...
    if reservation_item:
        ...
        key = {"shop_id": body["shopId"], "reserved_day": body["reservationDate"]}
        update_value = {
...
async def put_push_messages_to_dynamo(body, remind_date_difference):
    """
    ...
    message_id1 = await message_table_controller.put_push_message(
    ...
    message_id2 = await message_table_controller.put_push_message(
        body["userId"], CHANNEL_ID, flex_message_day_before, remind_date_day_before
    )
    logging.info(message_id2)

    return message_id1, message_id2


def put_push_messages_to_line(body):
    """
    プッシュメッセージのメッセージ情報を作成し、DynamoDBに登録する。
    DynamoDBへの登録処理自体は共通処理にて行っている。
    ...
    """
    # 当日のリマインドメッセージを登録
    flex_message = create_flex_message(body, 1)
    channel_info = channel_access_token_table_controller.get_item(CHANNEL_ID)
    line.send_push_message(
        channel_info["channelAccessToken"],
        flex_message,
        body["userId"],
    )
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # pushメッセージをDynamoに保存
        message_id1, message_id2 = asyncio.run(
    ...
    return utils.create_success_response(json.dumps({"reservationId": reservation_id}))
