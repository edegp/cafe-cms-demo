...
class RestaurantParamCheck(ParamCheck):
    def __init__(self, params):
        ...
        self.preferred_year_month = (
            params["preferredYearMonth"] if "preferredYearMonth" in params else None
        )  # noqa:E501
        self.preferred_day = (
            params["preferredDay"] if "preferredDay" in params else None
        )  # noqa:E501
        self.access_token = (
            params["accessToken"] if "accessToken" in params else None
        )  # noqa: E501
        self.course_id = params["courseId"] if "courseId" in params else None
        self.reservation_date = (
            params["reservationDate"] if "reservationDate" in params else None
        )  # noqa:E501
        self.reservation_starttime = (
            params["reservationStarttime"] if "reservationStarttime" in params else None
        )  # noqa:E501
        self.reservation_endtime = (
            params["reservationEndtime"] if "reservationEndtime" in params else None
        )  # noqa:E501
        self.reservation_people_number = (
            params["reservationPeopleNumber"]
            if "reservationPeopleNumber" in params
            else None
        )  # noqa:E501
        self.course_name = (
            params["courseName"] if "courseName" in params else None
        )  # noqa: E501
    ...
    def check_api_reservation_get(self):
    ...
    def check_api_reservation_delete(self):
        self.check_access_token()
        self.check_reservation_date()
        self.check_reservation_starttime()
    ...
    def check_preferred_year_month(self):
        if error := self.check_required(
            self.preferred_year_month, "preferredYearMonth"
        ):
            self.error_msg.append(error)
            return

        if error := self.check_year_month(
            self.preferred_year_month, "preferredYearMonth"
        ):
            self.error_msg.append(error)
    ...
    def check_preferred_day(self):
        ...
        if error := self.check_year_month_day(self.preferred_day, "preferredDay"):
            self.error_msg.append(error)
    ...
    def check_access_token(self):
        ...
        if error := self.check_length(self.access_token, "accessToken", 1, None):
            self.error_msg.append(error)
    ...
    def check_shop_id(self):
        ...
        if error := self.check_length(self.shop_id, "shopId", 1, None):
            self.error_msg.append(error)
    ...
    def check_reservation_date(self):
        if error := self.check_required(
            self.reservation_date, "reservationDate"
        ):  # noqa 501
            self.error_msg.append(error)
            return

        if error := self.check_year_month_day(
            self.reservation_date, "reservationDate"
        ):  # noqa 501
            self.error_msg.append(error)
    ...
    def check_reservation_starttime(self):
        if error := self.check_required(
            self.reservation_starttime, "reservationStarttime"
        ):  # noqa 501
            self.error_msg.append(error)
            return

        if error := self.check_time_format(
            self.reservation_starttime, "reservationStarttime", "%H%M"
        ):  # noqa 501
            self.error_msg.append(error)
            return
    ...
    def check_reservation_endtime(self):
        if error := self.check_required(
            self.reservation_endtime, "reservationEndtime"
        ):  # noqa 501
            self.error_msg.append(error)
            return

        if error := self.check_time_format(
            self.reservation_endtime, "reservationEndtime", "%H%M"
        ):  # noqa 501
            self.error_msg.append(error)
            return
    ...
    def check_reservation_people_number(self):
        if error := self.check_required(
            self.reservation_people_number, "reservationPeopleNumber"
        ):  # noqa 501
            self.error_msg.append(error)
            return

        if error := self.check_int(
            self.reservation_people_number, "reservationPeopleNumber"
        ):
            self.error_msg.append(error)
