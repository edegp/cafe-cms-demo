...
class DynamoDB:
    ...
    def _put_item(self, item):
        ...
        """
        try:
            response = self._table.put_item(Item=self._replace_data_for_dynamodb(item))
    ...
    def _query_index_filter(
        self, index, expression, filter_expression, expression_value
    ):
        """
        indexからアイテムを取得する
